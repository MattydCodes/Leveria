...
void openinventory(){
      ...
      if(mouseX < 250+x*220+40 && mouseX > 250+x*220-40 && mouseY > 290+y*220 && mouseY < 290+y*220+80){
        if(inventory[x][y].empty == false){
          ...
          if(inventory[x][y].weapon){
            ...
            ui.text("Value : " + int((inventory[x][y].stat)/(inventory[x][y].cdreset)*2) + "SF",255+x*220+40,380+y*220);
          }else{
  ...
  if(mouseX < 1500+40 && mouseX > 1500-40 && mouseY > 780 && mouseY < 780+80){
    ...
    if(mousePressed && cused == false && inhand.empty == false){
      if(inhand.weapon){
        points+=int((inhand.stat)/(inhand.cdreset)*2);
      }else{
  ...
  if(inforge.empty == false){
    if(inforge.weapon){
      cost=int((inforge.stat+damagec*damagec)/(inforge.cdreset-cdc)*2*(1+cdc*5));
    }else{
      cost=int((armourc*armourc+inforge.stat)*10);
    }
